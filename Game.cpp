...
// Hack functions
Game::Game() : gpu{ Graphics::Graphics(this) } {}

void Game::HandleTriggerbot() {
...
void Game::DrawESP() {
	for (int i = 1; i < 32; i++) {
		float VMatrix[16];
		modClient.GetViewMatrix(VMatrix); // initialize VMatrix
		try {
			...
			headPos.z += 8;
			if (!gpu.WorldToScreen(headPos, headPos2D, VMatrix)) throw "W2S Error.";
			if (!gpu.WorldToScreen(playerPos, playerPos2D, VMatrix)) throw "W2S Error.";
